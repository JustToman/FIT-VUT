...
**/
	...
	if((*RootPtr)->RPtr != NULL)
	{
		if((*RootPtr)->RPtr->RPtr == NULL)
		{
			...
			tBSTNodePtr new_right = (*RootPtr)->RPtr->LPtr;
			free((*RootPtr)->RPtr);
			(*RootPtr)->RPtr = new_right;
		}
		else
	...
	else
	{
		...
		tBSTNodePtr new_right = (*RootPtr)->LPtr;
		free((*RootPtr));
		(PtrReplaced)->LPtr = new_right;
	}
...
**/
	...
	else if(K == (*RootPtr)->Key)
	{
		...
		else
		{
			...
			{
				if(daddy->RPtr == node)
					daddy->RPtr = node->LPtr;
				else
					daddy->LPtr = node->LPtr;
			...
			{
				if(daddy->RPtr == node)
					daddy->RPtr = node->RPtr;
				else
					daddy->LPtr = node->RPtr;
				free(node);
			}
