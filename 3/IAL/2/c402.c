...
**/
	tBTNodePtr node;
	tBTNodePtr daddy = NULL;
	for(tBTNodePtr node = *RootPtr; node != NULL;)
	{
		daddy = node;
	...
	node = (tBTNodePtr) malloc(sizeof(struct tBTNode));
	node->Cont = Content;
	node->LPtr = NULL;
	node->RPtr = NULL;
...
**/
	...
	do
	{
		while((*RootPtr) != NULL)
		{
			SPushP(&stack_left,(*RootPtr));
			prev_node = (*RootPtr);
			(*RootPtr) = (*RootPtr)->LPtr;
		}
		(*RootPtr) = prev_node ->RPtr;
	  
		if((*RootPtr) == NULL)
		{
			tBTNodePtr to_be_deleted = STopPopP(&stack_left);
			if(!SEmptyP(&stack_left))
			{
			(*RootPtr) = STopPopP(&stack_left);
			if(to_be_deleted == (*RootPtr) -> LPtr)
				(*RootPtr) -> LPtr = NULL;
			else
				(*RootPtr) -> RPtr = NULL;
			}
	...
	}while((!SEmptyP(&stack_left)) || *(RootPtr) != NULL);
